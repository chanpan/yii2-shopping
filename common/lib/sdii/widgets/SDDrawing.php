...
use common\lib\sdii\assets\drawing\DrawingAsset;
use yii\helpers\Url;
use yii\helpers\Html;
use dosamigos\fileupload\FileUpload;
use Yii;

class SDDrawing extends InputWidget
{
    public $ui_temp = '<div id="{canvasDraw}" class="canvasDraw">
			    <div class="text-center drawingContent">
					<div class="drawingToo btn-toolbar" role="toolbar">
						<div class="row"><div class="btn-group-vertical" data-toggle="buttons">
						...
						<br>
						<div class="row"><div class="btn-group-vertical">
				...
				<div id="{canvasDraw}_box" class="canvasDraw_box"></div>
			</div>
	...
	if(!isset($this->options['saveUrl'])){
	    $this->options['saveUrl'] = Url::to(['//ezforms/drawing/save-image']);
	} 
	...
	if(!isset($this->options['bgUrl'])){
	    ...
	    $this->options['bgUrl'] = Url::to(['//ezforms/drawing/bg-image', 'name'=>$inputID]);
	}
	...
	if(isset($inputValue) && $inputValue!=''){
		...
		if(count($fileArr)>1){
			...
			if(stristr($fileName, '.png') == TRUE){
				...
				$nowFileName = $idName . '_'.$inputID.'_tmp.png';

				@copy(Yii::$app->basePath . '/../storage/web/drawing/data/' . $fileName, Yii::$app->basePath . '/../backend/web/drawing/' . $nowFileName);

				$this->options['outlineName'] = $nowFileName;
				$this->options['outlinePath'] = Yii::getAlias('@backendUrl').'/drawing';
			}
		}
	}
	//echo $this->ui_temp;
	...
	if (isset($this->default_bg) && $this->default_bg!='') {
	    ...
	    if(stristr($fileBg, '.png') == TRUE){
			$view = $this->getView();
			list($width, $height, $type, $attr) = getimagesize(Yii::$app->basePath . '/../storage/web/drawing/bg/'.$fileBg);
			...
			$view->registerJs("
				...
				$('#{$this->options['drawingName']}').css('background-image', 'url(".Url::to(Yii::getAlias('@storageUrl').'/drawing/bg/'.$fileBg).")');
				$('#{$this->options['drawingName']}').css('background-size', bgsize);
				$('#{$this->options['drawingName']}').css('background-position', 'center center');
				$('#{$this->options['drawingName']}').css('background-repeat', 'no-repeat');
			");
	...
	$view->registerJs("
	    ...
	    $('#".$this->options['saveBtn']."').click(function(){
			...
			data: {type: 'data', image: img, name: '".$inputID."'},
			dataType: 'JSON',
